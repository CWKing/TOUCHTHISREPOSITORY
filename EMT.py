...
#---------------------------------------------------------------------------
#Update the dictionaries with these interpolations
...
dictUpdate(AmorphCarb, interper(AmorphCarb['WAV'], AmorphCarb['K']), 'K')
...
dictUpdate(AstroSil, interper(AstroSil['WAV'], AstroSil['K']), 'K')
...
dictUpdate(Water, interper(Water['WAV'], Water['K']), 'K')
...
#--------------------------------------------------------------
#Finally doing what this program is named after! Yay!
#Initialize the Optical Constants dictionary
IMPOptConst = {'WAV':Kates['WAV'], 'N':[], 'K':[]}

#Iterate through the reals and complexes and utilize EMT to generate new
#optical constants
i = 0
while i < len(Water['N']):
    
    #Utilize EMT for Water (Matrix) and Amorphous Carbon (Inclusion)
    N, K = EMT(Water['N'][i], Water['K'][i], AmorphCarb['N'][i], AmorphCarb['K'][i])
    
    #Utilize the results from the last utilization of EMT for this run of EMT
    #now with AstroSil (Inclusion)
